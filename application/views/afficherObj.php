				...
				<div class="card w-100 d-flex flex-row">
					<div class="col-md-6">
						<?php foreach($this->objet->getAllImages($current_objet->getidObjet()) as $image) { ?>
							<img class="card-img-top rounded" src="<?php echo site_url('assets/images/'.$image); ?>" alt="Card image cap">
						<?php } ?>
					</div>
					...
					<h1 class="card-title">Nom Objet : <?php echo $current_objet->getnomObjet(); ?></h3>
						...
						<a class="btn btn-primary mt-3" href="<?php echo site_url('frontoffice/echange/echange/'.$current_objet->getidObjet()); ?>">Echanger</a>
						<?php if (isset($error)) ?>
							<span>
